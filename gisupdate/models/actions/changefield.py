...
class changestate():
    def __init__(self, nombre_arg, valor_arg, id_arg, tabla):
        print (("LA TABLA ES ESTADO - ID : ", str(nombre_arg), str(id_arg)))
        #self._state_name = estado_arg
        self._field_name = nombre_arg
        self._field_valor = valor_arg
        self._id_reg = id_arg
        self._tabla = tabla
    ...
    def changeState(self):
        print (("LA VARIABLE DENTRO DE FUNCION ES: "+ str(self._field_name)))
        print (("el ID DEL REGISTRO A MODIFICAR ES: "+ str(self._id_reg)))
        print (("TABLA A MODIFICAR ES: "+ str(self._tabla)))
        field_selec = self._field_name
        value_selec = self._field_valor
        ...
        #self._conectar()
        try:
            query = "UPDATE "+tabla_selec+" SET "+ field_selec +" = '" + value_selec+ "' WHERE id = " + str(id_selec)
...
id_reg = sys.argv[3]
tabla = sys.argv[4]
u = changestate(nombre, valor, id_reg, tabla)
u.changeState()